...
object YouTube {
    ...
    suspend fun podcastWithDebug(podcastId: String, log: (String) -> Unit): Result<PodcastPage> = runCatching {
        ...
        header?.buttons?.forEachIndexed { i, button ->
            Logger.d("[PODCAST] Button[$i]: menuRenderer=${button.menuRenderer != null}, toggleButtonRenderer=${button.toggleButtonRenderer != null}, playButtonRenderer=${button.musicPlayButtonRenderer != null}")
            button.menuRenderer?.items?.forEachIndexed { j, item ->
                Logger.d("[PODCAST] Button[$i].menuItems[$j]: toggle=${item.toggleMenuServiceItemRenderer?.defaultIcon?.iconType}, nav=${item.menuNavigationItemRenderer?.icon?.iconType}")
            ...
            button.toggleButtonRenderer?.let { toggle ->
                Logger.d("[PODCAST] Button[$i].toggleButtonRenderer: defaultIcon=${toggle.defaultIcon?.iconType}, defaultToken=${toggle.defaultServiceEndpoint?.feedbackEndpoint?.feedbackToken?.take(30)}, subscribeChannelIds=${toggle.defaultServiceEndpoint?.subscribeEndpoint?.channelIds}")
            }
        }
            ...
            header?.buttons?.forEach { button ->
                button.toggleButtonRenderer?.let { toggle ->
                    ...
                    if (iconType != null && PageHelper.isLibraryIcon(iconType)) {
                        ...
                        Logger.d("[PODCAST] Found toggle button with library tokens - add: ${libraryTokens.addToken != null}, remove: ${libraryTokens.removeToken != null}")
                    }
                }
            }
        }
        ...
        secondaryContents?.sectionListRenderer?.contents?.forEachIndexed { index, content ->
            Logger.d("Content[$index]: musicShelfRenderer=${content.musicShelfRenderer != null}, musicPlaylistShelfRenderer=${content.musicPlaylistShelfRenderer != null}, gridRenderer=${content.gridRenderer != null}")
    ...
    suspend fun newEpisodes(): Result<List<SongItem>> {
        ...
        return runCatching {
            ...
            result.onSuccess { Logger.d("[PODCAST_API] newEpisodes SUCCESS: ${it.size} items") }
        }
    }
